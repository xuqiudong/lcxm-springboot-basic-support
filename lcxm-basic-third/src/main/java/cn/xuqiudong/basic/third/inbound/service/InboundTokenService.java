...
public class InboundTokenService {
     ...
     * 校验第三方签名并签发 token。
     */
    public TokenIssueResult issueToken(TokenApplyRequest request) {
        String invalidMessage = validateBasicRequest(request);
        if (invalidMessage != null) {
            return TokenIssueResult.failed(invalidMessage);
        }
        ...
        if (!verified) {
            return TokenIssueResult.failed("invalid sign");
        }
        if (config.isNonceRequired() && nonceStore != null) {
            ...
            if (!saved) {
                return TokenIssueResult.failed("repeated nonce");
            }
        }
        ...
        return TokenIssueResult.success(token);
    }
    ...
    public TokenCheckResult checkToken(String token) {
        if (StrUtil.isBlank(token)) {
            return TokenCheckResult.failed("blank token");
        }
        TokenValue tokenValue = tokenStore.get(token);
        if (tokenValue == null) {
            return TokenCheckResult.failed("invalid token");
        }
        if (tokenValue.isExpired()) {
            tokenStore.remove(token);
            return TokenCheckResult.failed("expired token");
        }
        return TokenCheckResult.success(tokenValue);
    }

    /**
     * 主动注销 token。
     */
    public TokenCheckResult revokeToken(String token) {
        TokenCheckResult result = checkToken(token);
        if (!result.isSuccess()) {
            return result;
        }
        tokenStore.remove(token);
        return TokenCheckResult.success(result.getTokenValue());
    }
