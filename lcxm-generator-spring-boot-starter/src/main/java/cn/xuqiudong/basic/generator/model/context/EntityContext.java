...
public class EntityContext extends BaseContext {
     ...
     * 实体类信息  上下文
     */
    public EntityContext(TableInfo tableInfo, ConfigBundle bundle) {
        super(tableInfo, bundle, bundle.getStrategyConfig().getEntityTemplateConfig());
        // 主键类型 暂只支持 ASSIGN_UUID 和  AUTO(mysql  自增)
        if (tableInfo.getPk().getMeta().isAutoIncrement()) {
            this.idType = IdType.AUTO;
        }
        // 把未忽略的字段加到 fields 中
        this.fields = tableInfo.getXmlFields().stream().filter(field -> !field.isEntityIgnore()).toList();
        // 字段对应的数据类型是否需要导入
    ...
    public List<Class<?>> genericClassList(TableInfo tableInfo) {
        return List.of(tableInfo.getPk().getDataType().getJavaType());
    }
